        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                case "get_project_status": {
                    const projectPath = request.params.arguments?.path || process.cwd();
                ...
                case "propose_environment_fix": {
                    const error = request.params.arguments?.errorMessage;
                    let suggestion = "Try running 'envsetup doctor' to check common issues.";
                    if (error.includes("docker"))
                        suggestion = "Docker seems to be the issue. Ensure the Docker daemon is running.";
                    if (error.includes("port 3000"))
                        suggestion = "Port 3000 is occupied. Change the mapping in docker-compose.yml.";
                    return {
