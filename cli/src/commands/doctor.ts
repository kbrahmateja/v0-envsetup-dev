...
export async function doctorCommand() {
  ...
  const results: { name: string; status: 'ok' | 'error'; version?: string; error?: string }[] = [];
  ...
  results.forEach((r) => {
    if (r.status === 'ok') {
      ...
      if (r.error) console.log(pc.dim(`  Error: ${r.error}`));
    }
  });
...
function checkDependency(command: string, name: string, results: any[]) {
  try {
